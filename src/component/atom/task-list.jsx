...
const GenerateTask = ({ task, stateTask, index, dailytaskIndex, fetchData, claimStateList, setClaimStateList, disableList, setDisableList, moneadshow }) => {
  ...
  return (
    <div className={`${task.highLight == 1 && task.status === 1 ? "bg-[url('/image/task-bg.png')] border-2 border-[#FFD700]" : "bg-[#0000001A] border-0"} bg-cover rounded-lg flex justify-between items-center gap-2 py-2 pl-2 pr-4 text-[14px]`}>
      <div className="flex gap-2 items-center">
        <img src={task.src} alt="" className="w-8 h-8 rounded-full" />
        <div className="flex flex-col">
          <div className={`${task.highLight == 1 && task.status === 1 ? "text-[#FAE66C]" : "text-white"}`}>{task.title}</div>
          <div className={`${task.highLight == 1 && task.status === 1 ? "text-[#FAE66C99]" : "text-[#ffffff99]"} w-[210px]`}>
            +{
              ...
              (task.link === null || task.link === "") ?
                (
                  (task.index === 25 || task.index === 26 && !wallet) ?
                    <Link to={'/wallet'}>
                      <button className={`${task.highLight == 1 && task.status === 1 ? "bg-mainYellow text-main" : "bg-mainFocus text-white"} rounded-lg w-[61px] py-1 px-0 h-7 text-center text-[14px]`} >
                        Start
                      </button>
                    </Link> :
                    (
                      (task.index === 26 && wallet) ?
                        <button className={`rounded-lg w-[61px] py-1 px-0 h-7 ${task.highLight == 1 && task.status === 1 ? "bg-mainYellow text-main" : "bg-mainFocus text-white"} text-center text-[14px]`} onClick={() => sendTransaction(500)} >
                          Start
                        </button> :
                        <Link to={'/play'}>
                          <button className={`${task.highLight == 1 && task.status === 1 ? "bg-mainYellow text-main" : "bg-mainFocus text-white"} rounded-lg w-[61px] py-1 px-0 h-7 text-center text-[14px]`} >
